...
export const LightWalletButton = ({
  variant = 'header',
  className
}: LightWalletButtonProps) => {
  const {
    address,
    isConnected
  } = useAccount();
  ...
  useEffect(() => {
    if (user) {
      ...
      const channel = supabase.channel('wallet-balance').on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'profiles',
  ...
  const fetchNftAvatar = async () => {
    ...
    const {
      data
    } = await supabase.from('fun_id').select('soul_nft_id, avatar_glow_color').eq('user_id', user.id).single();
    if (data?.soul_nft_id) {
  ...
  if (variant === 'floating') {
    return <>
        <motion.button onClick={() => setIsModalOpen(true)} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)} className={cn("fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-2 rounded-full", "bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500", "border-2 border-yellow-300 shadow-lg shadow-amber-500/30", "hover:shadow-xl hover:shadow-amber-500/50 transition-all duration-300", className)} whileHover={{
        scale: 1.05
      }} whileTap={{
        scale: 0.95
      }}>
          ...
          <AnimatePresence>
            {isHovered && <>
                {[...Array(5)].map((_, i) => <motion.div key={i} className="absolute" initial={{
              opacity: 0,
              scale: 0
            }} animate={{
              opacity: [0, 1, 0],
          ...
          {isConnected ? <div className="flex items-center gap-2">
              {nftAvatarUrl && <img src={nftAvatarUrl} alt="Soul NFT" className="w-6 h-6 rounded-full border-2 border-white" />}
              <span className="font-bold text-white text-sm">
                {formatCamly(camlyBalance)} CAMLY
              </span>
            </div> : <span className="font-bold text-white text-sm">Connect Wallet</span>}

          <Sparkles className="w-4 h-4 text-white animate-pulse" />
        </motion.button>

        <LightWalletModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} camlyBalance={camlyBalance} onBalanceUpdate={fetchCamlyBalance} />
      </>;
  }

  // Header variant
  return <>
      <motion.button onClick={() => setIsModalOpen(true)} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)} className={cn("relative flex items-center gap-2 px-4 py-2 rounded-xl", "bg-gradient-to-r from-amber-500/20 to-yellow-500/20", "border border-amber-500/30 hover:border-amber-500/60", "transition-all duration-300 group", className)} whileHover={{
      scale: 1.02
    }} whileTap={{
      scale: 0.98
    }}>
        ...
        <AnimatePresence>
          {isHovered && <>
              {[...Array(3)].map((_, i) => <motion.div key={i} className="absolute" initial={{
            opacity: 0,
            scale: 0
          }} animate={{
            opacity: [0, 1, 0],
        ...
        {isConnected && address ? <div className="flex items-center gap-2">
            {nftAvatarUrl}
